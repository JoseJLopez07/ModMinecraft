...
package net.mcreator.virgencraft.entity;
...
public class ShurikenEntity extends AbstractArrow implements ItemSupplier {
	public ShurikenEntity(PlayMessages.SpawnEntity packet, Level world) {
		super(VirgencraftModEntities.SHURIKEN.get(), world);
	}
	...
	public ItemStack getItem() {
		return new ItemStack(VirgencraftModItems.SHIRKEN_ITEM.get());
	}
	...
	public static ShurikenEntity shoot(Level world, LivingEntity entity, Random random, float power, double damage, int knockback) {
		ShurikenEntity entityarrow = new ShurikenEntity(VirgencraftModEntities.SHURIKEN.get(), entity, world);
	...
	public static ShurikenEntity shoot(LivingEntity entity, LivingEntity target) {
		ShurikenEntity entityarrow = new ShurikenEntity(VirgencraftModEntities.SHURIKEN.get(), entity, entity.level);
		double dx = target.getX() - entity.getX();
		double dy = target.getY() + target.getEyeHeight() - 1.1;
		double dz = target.getZ() - entity.getZ();
		entityarrow.shoot(dx, dy - entityarrow.getY() + Math.hypot(dx, dz) * 0.2F, dz, 1f * 2, 12.0F);
		entityarrow.setSilent(true);
		entityarrow.setBaseDamage(4.9);
